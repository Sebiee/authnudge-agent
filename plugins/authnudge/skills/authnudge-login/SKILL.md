...
- Computer-use or a Chrome you start yourself: start it with `--remote-debugging-port=9222` (add `--user-data-dir` if you want a separate profile). Then drive it with computer-use as usual.
- Playwright: launch Chromium with the arg `--remote-debugging-port=9222`, or connect Playwright over CDP to a Chrome you started that way. Pass a different port as `cdpUrl` if needed.
